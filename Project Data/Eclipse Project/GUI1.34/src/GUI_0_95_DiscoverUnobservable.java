...
public class GUI_0_95_DiscoverUnobservable extends PApplet {
	PImage robot, dirt;
	int x, y;
	Room start;
	...
	AgentUnobservableDiscover agent;
	int appTime = -1, fc = -1;
	Timer t;
	...
	public void setup() {
		...
		surface.setTitle("BFS to shortest Dirt Agent");
		surface.setIcon(loadImage("icon.png"));
		start = new Room(x, y);
		createGUI();
		agent = new AgentUnobservableDiscover(0, 0, 100000, start);
		startTimer();
	}
	...
	public void startTimer() {
		...
		t.schedule(new TimerTask() {
			...
			public void run() {
				...
				surface.setTitle("[Unoservable Environement Agents] Dicover the Room Agent " + nf(appTime / 60, 2)
	...
	public void draw() {
		...
		background(209);
		k = (float) Math.min(((height - 1.0) / (2 * y + 1)), (width - 150.0) / x);
		...
		for (int i = 0; i < x; i++) {
			for (int j = 0; j < y; j++) {
				if (agent.visited[i][j]) {
					fill(255);
					rect(i * k, (j + y + 1) * k, k, k);

					if (!agent.agentRoom.grid[i][j].isClean()) {
						if (k < 5) {
							fill(200, 200, 0);
							rect(i * k, (j + y + 1) * k, k, k);
						} else {
							...
							kold = k;
							image(dirt, i * k, (j + y + 1) * k);
						}
					}

				}
				if (agent.visited[i][j]) {
					if (agent.agentRoom.grid[i][j].hasWalls()) {
						fill(0, 0, 0);
						float wallWidth = (float) (k / 26.0);
						if (agent.agentRoom.grid[i][j].HasWallUp())
							rect(i * k, (j + y + 1) * k, k, wallWidth);
						if (agent.agentRoom.grid[i][j].HasWallDown())
							rect(i * k, (j + y + 2) * k - wallWidth, k, wallWidth);
						if (agent.agentRoom.grid[i][j].HasWallLeft())
							rect(i * k, (j + y + 1) * k, wallWidth, k);
						if (agent.agentRoom.grid[i][j].HasWallRight())
							rect((i + 1) * k - wallWidth, (j + y + 1) * k, wallWidth, k);
					}
				}
			}
		}
		...
		// arc((o + 0.5f) * k, (u + 0.5f) * k, k * 0.9f, k * 0.9f, QUARTER_PI / 2, PI +
		// 7 * QUARTER_PI / 2);
		image(robot, agent.x * k, (agent.y + y + 1) * k);
		...
		textSize(14.5f);
		text("Agent: " + (Run ? idle ? "Idle" : "On" : "Off"), (float) (width - 142), val);
		...
		text("Time: " + nf(agent.timeOn / 60, 2) + ":" + nf(agent.timeOn == -1 ? 0 : agent.timeOn % 60, 2),
	...
	public void Run() {
		...
		if (start.isDirt(agent.x, agent.y)) {
			...
			// System.out.println(agent.AllScanned());
			if (!agent.AllScanned()) {
				if (agent.queue.isEmpty()) {
					if (!agent.visited[agent.x][agent.y])
						agent.queue.add(agent.agentRoom.grid[agent.x][agent.y]);
					// agent.visited[agent.x][agent.y] = true;
				}
				if (!agent.updated || !agent.queue.isEmpty()) {
					// System.out.println(agent.queue);
					if (agent.updated)
						agent.current = agent.queue.remove(0); // Dequeue current index
					// current = agent.agentRoom[agent.x][agent.y];
					// agent.agentRoom.grid[agent.x][agent.y] = start.grid[agent.x][agent.y];
					if (start.grid[agent.x][agent.y].hasWalls()) {
						if (start.grid[agent.x][agent.y].HasWallUp())
							agent.agentRoom.putWallUp(agent.x, agent.y);
						if (start.grid[agent.x][agent.y].HasWallDown())
							agent.agentRoom.putWallDown(agent.x, agent.y);
						if (start.grid[agent.x][agent.y].HasWallLeft())
							agent.agentRoom.putWallLeft(agent.x, agent.y);
						if (start.grid[agent.x][agent.y].HasWallRight())
							agent.agentRoom.putWallRight(agent.x, agent.y);

					}
					if (!agent.visited[agent.current.getX()][agent.current.getY()] || !agent.updated) {
						agent.visited[agent.current.getX()][agent.current.getY()] = true;
						// System.out.println("Test"+agent.visited[agent.current.getX()][agent.current.getY()]);
						if (!agent.AllScanned()) {
							agent.updated = false;
							for (Tile tile : agent.agentRoom.getNeighbors(agent.current)) {
								if (!agent.visited[tile.getX()][tile.getY()] && !agent.updated) {
									// agent.visited[tile.getX()][tile.getY()] = true;
									agent.queue.add(tile);
									agent.prev[tile.getX()][tile.getY()] = agent.current;
									updateAgent(agent.queue.get(0).getX(), agent.queue.get(0).getY());
									agent.updated = true;
								}
							}
							// System.out.println(agent.updated + " " + agent.queue.size());
							if (!agent.updated) {
								// System.out.println("A");
								agent.current = agent.prev[agent.current.getX()][agent.current.getY()];
								// if(agent.queue.get(0)==current)
								// updated = true;
								updateAgent(agent.current.getX(), agent.current.getY());
	...
	public void reset() {
		...
		agent.steps = 0;
		appTime = -1;
		agent.visited = new boolean[agent.agentRoom.getGridWidth()][agent.agentRoom.getGridHeight()];
		agent.prev = new Tile[agent.agentRoom.getGridWidth()][agent.agentRoom.getGridHeight()];
		agent.queue = new ArrayList<>();
		agent.updated = true;
		agent.current = new Tile(0, 0);
		loop();
	}
	...
	public void outputFileSelected(File selection) {
		if (selection == null) {
			...
			start.save(selection.getAbsolutePath() + ".aroom");
		}
	}
	...
	static public void main(String[] passedArgs) {
		String[] appletArgs = new String[] { "GUI_0_95_DiscoverUnobservable" };
		if (passedArgs != null) {
