...
public class GUI_0_95_SA_TSP_Unobservable extends PApplet {
	PImage robot, dirt;
	int x, y;
	Room start;
	...
	public void setup() {
		...
		pWidth = 0;
		pHeight = 0;
		x = 4; // default height and width of the room
		y = 3;
		...
		start = new Room(x, y);
		AgentRoom = start.copy();
		AgentRoom.addDirtToAll();
		agent = new AgentUnobservable(0, 0, 100000, AgentRoom);
		createGUI();
		startTimer();
...
//							agent.state = agent.defaultState;
				...
				surface.setTitle("[Unoservable Environement Agents] TSP Agent " + nf(appTime / 60, 2) + ":"
		...
		for (int i = 0; i < x; i++) {
			for (int j = 0; j < y; j++) {
				...
				if(showBox.isSelected())
					if(!results.isEmpty()&&!results.contains(agent.agentRoom.grid[i][j]))
						fill(150,255,150);
				rect(i * k, j * k, k, k);
	...
	public void Run() {
		try {
			if (start.isDirt(agent.x, agent.y)) {
				...
				Tile tile;
				if (pause && results.isEmpty()) {
					pause = false;
					// long t1 = System.currentTimeMillis();
//					System.out.println("R" + agent.road);
					if (tspResults.size() <= 1 && agent.road.isEmpty()) {
						int temp = 100000;
						double ratio = 0.0001;
						...
						int copyReset = reset;
						agent.agentRoom = start.copy();
						agent.agentRoom.addDirtToAll();

						ArrayList<Tile> arl = SA_TSP.solve(agent.agentRoom, agent, temp, ratio);
//						System.out.println(arl);
						if (reset == copyReset) // the result above may take lots of time, this reset is used to not
												// take it after computation in case
							tspResults = arl; // resets occurred in middle of a computation; reset can be detected
												// if reset!copyReset befor computation

					}
					if (agent.road.isEmpty()) {
						agent.road = new ArrayList<>(tspResults);
						Collections.reverse(agent.road);
//						System.out.println(agent.road);
						agent.nbOfDirtIteration=0;
					} else {
						if (tspResults.isEmpty()) {
							tspResults = new ArrayList<>(agent.road);
							Collections.reverse(agent.road);
//							System.out.println("NB"+agent.nbOfDirtIteration);
...
//					}
					...
					// System.out.println(tspResults.size());
					 if (tspResults.size() > 1) {
					 results = new ArrayList<>(tspResults);
...
//					 System.out.println(results);
	...
	public void createGUI() {
		...
		showBox = new GCheckbox(this, 356, 136, 120, 30, "Show Visited");
	...
	GCheckbox roomBox, dirtBox, wallsBox, showBox;
	...
	static public void main(String[] passedArgs) {
		String[] appletArgs = new String[] { "GUI_0_95_SA_TSP_Unobservable" };
		if (passedArgs != null) {
