...
public class GUI_0_95_PartiallyObservableA extends PApplet {
	PImage robot, dirt;
	int x, y;
	int vis;
	Room start;
	...
	AgentPartiallyObservable agent;
	int appTime = -1, fc = -1;
	Timer t;
	...
	public void setup() {
		...
		start = new Room(x, y);
		vis = (int) Math.max(x, y);
		createGUI();
		agent = new AgentPartiallyObservable(0, 0, 100000, start.getGridWidth(), start.getGridHeight(), vis);
		startTimer();
	}
	...
	public void startTimer() {
		...
		t.schedule(new TimerTask() {
			...
			public void run() {
				...
				if (Run) {
					if (!idle) {
						...
						if (agent.countdown-- < 0) {
							// agent.state = agent.defaultState;
							idle = false;
							agent.visited = new boolean[start.w][start.h];
						}
					}
					if (agent.battery <= 0) {
						agent.battery = 0;
				...
				surface.setTitle("[Parially Observable Environement Agents] Weights Oriented Agent "
	...
	public void Run() {
		if (!idle) {
			if (start.isDirt(agent.x, agent.y)) {
				...
				agent.nbOfDirtIteration++;
			}
			if (agent.AllScanned(start)) {
				if (agent.nbOfDirtIteration == 0)
					agent.state++;
				else
					agent.resetState();
				agent.nbOfDirtIteration=0;
				agent.visited = new boolean[start.getGridWidth()][start.getGridHeight()];
				if (agent.state >= 0) {
					idle = true;
					agent.pause();
				}
				...
				Tile tile;
				ArrayList<Tile> steps;
				if (results == null || results.isEmpty()) {
					results = AgentPartiallyObservable.BFS.compute(start, agent, false);
					System.out.println("1: " + results);
					agent.displayDiscovered();
					System.out.println();
					if (results == null) {
						results = new ArrayList<>();
						steps = AgentPartiallyObservable.BFS2.compute(start, agent, true);
						// System.out.println("2: " + steps);
						Tile nextTile = steps.get(1);
						results.add(nextTile);
						// results=AgentPartiallyObservable.BFS2.compute(start, agent, false);
						// System.out.println("2 "+results);
						agent.visited[nextTile.getX()][nextTile.getY()] = true;
	...
	public void reset() {
		...
		appTime = -1;
		// agent.visited = new
		// boolean[agent.agentRoom.getGridWidth()][agent.agentRoom.getGridHeight()];
		// agent.prev = new
		// Tile[agent.agentRoom.getGridWidth()][agent.agentRoom.getGridHeight()];
	...
	public void submit_click(GButton source, GEvent event) {
		try {
			...
			background(209);
			agent = new AgentPartiallyObservable(0, 0, 10000, start.getGridWidth(), start.getGridHeight(), vis);
			results = new ArrayList<>();
			start = new Room(x, y);
			reset();
		} catch (Exception e) {
	...
	public void set_vis(GButton source, GEvent event) {
		try {
		...
		if (selection == null) {
			...
			try {
				...
				if (r != null) {
					noLoop();
					start = r;
					agent = new AgentPartiallyObservable(0, 0, 100000, start.getGridWidth(), start.getGridHeight(),
							vis);
					x = start.getGridWidth();
					y = start.getGridHeight();
					results = new ArrayList<>();
	...
	public void createGUI() {
		...
		// visBtn= new GButton(this, 570, 68, 80, 26);
		visBtn = new GButton(this, 685, 390, 80, 26);
		visBtn.setText("Set Visibility");
		visBtn.setTextBold();
		visBtn.setLocalColorScheme(GCScheme.YELLOW_SCHEME);
		visBtn.addEventHandler(this, "set_vis");
		visBtn.fireAllEvents(true);
		// visField = new GTextField(this,570, 43, 80, 20, G4P.SCROLLBARS_NONE);
		visField = new GTextField(this, 685, 365, 80, 20, G4P.SCROLLBARS_NONE);
		visField.setPromptText("Set Vis");
		visField.setOpaque(false);
		visBox = new GCheckbox(this, 685, 415, 70, 30, "Show Vis");
	...
	GButton wallBtn, dirtBtn, agentLoc, submitBtn, startBtn, saveBtn, visBtn, loadBtn, submitB, generateBtn;
	GTextField xField, yField, batteryField, visField;
	GCheckbox roomBox, dirtBox, wallsBox, visBox;
	...
	static public void main(String[] passedArgs) {
		String[] appletArgs = new String[] { "GUI_0_95_PartiallyObservableA" };
		if (passedArgs != null) {
