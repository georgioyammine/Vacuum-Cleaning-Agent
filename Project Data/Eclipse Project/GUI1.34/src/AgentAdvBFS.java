...
public class AgentAdvBFS extends Agent {
	boolean identity = true;
	

	public AgentAdvBFS(int x, int y, int battery, boolean identity) {
		super(x, y, battery);
	...
	public boolean hasAgent(int x, int y, ArrayList<AgentAdvBFS> agents) {
		for (Agent a : agents)
	...
	public boolean hasAdvAgent(int x, int y, ArrayList<AgentAdvBFS> agents) {
		for (Agent a : agents)
			if ((identity ^ a.getIdentitity()) && a.getX() == x && a.getY() == y)
				return true;
		return false;
	}
	...
	public ArrayList<Tile> compute(Room room, Agent agent, boolean shuffle, ArrayList<AgentAdvBFS> agents) {
	...
	public ArrayList<Tile> BFStoAdv(Room room, Agent agent, boolean shuffle, ArrayList<AgentAdvBFS> agents) {
		Queue<Tile> queue = new LinkedList<>(); // create the LinkedList and using it as a queue
		Tile current = room.grid[agent.x][agent.y];
		prev = new Tile[room.getGridWidth()][room.getGridHeight()];
		queue.add(current); // enqueuing first Tile
