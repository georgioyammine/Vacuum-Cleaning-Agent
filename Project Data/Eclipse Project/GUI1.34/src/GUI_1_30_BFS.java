...
public class GUI_1_30_BFS extends PApplet {
	...
	public void resetAgents() {
		...
		Agents.add(new AgentAdvBFS(1, 0, 100000, false));
	}
		...
		try {
			...
			for (Agent agent : Agents) {
				...
				if (Agents.indexOf(agent) < ma) {
					...
					text("Steps: " + results[2], (float) (width - 142), val + 40);
...
//									Run = false;
	...
	public boolean isEmpty(Tile a, ArrayList<AgentAdvBFS> agents2) {
		x = a.getX();
		y = a.getY();
		for (Agent agent : Agents)
