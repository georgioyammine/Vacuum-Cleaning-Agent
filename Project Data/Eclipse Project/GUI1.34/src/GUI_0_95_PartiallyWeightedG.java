...
public class GUI_0_95_PartiallyWeightedG extends PApplet {
	PImage robot, dirt;
	int x, y;
	Room start;
	...
	public void setup() {
		...
		start = new Room(x, y);
		createGUI();
		agent = new AgentPartially(0, 0, 100000, start);
		startTimer();
	}
	...
	public void Run() {
		// put code here to update the agent
		agent.agentRoom = start;
		// System.out.println("A");
		int xold = agent.x, yold = agent.y;
		if(idle&&agent.hasDirtArround()) {
//			System.out.println();
			idle = false;
			agent.resetMemory();
//			System.out.println("e");
		...
		if (start.isDirt(agent.x, agent.y)) {
			...
			if (!agent.AllScanned()) {
				...
				Tile next = agent.getNextPos();
				for (Tile a : start.getNeighbors(start.grid[agent.x][agent.y])) {
					if (!start.isDirt(a.getX(), a.getY()))
						agent.visited[a.getX()][a.getY()] = true;
				}
				// System.out.println("B"+agent.getNextPos());
				if (!agent.AllScanned()) {
					// System.out.println(next);
					int a = next.getX();
					int b = next.getY();
//					System.out.println(a + " " + b);
					// updateAgent(next.getX(), agent.getY());
					updateAgent(a, b);

					// for (int i = 0; i < start.h; i++) {
					// for (int j = 0; j < start.w; j++) {
					// System.out.print(agent.visited[j][i] ? "1 " : "0 ");
					// }
					// System.out.println();
					// }
				}
	...
	public void start_click(GButton source, GEvent event) {
		...
		if (event == GEvent.PRESSED) {
			...
			if (Run) {
				...
				startBtn.setTextBold();
				agent = new AgentPartially(agent.x, agent.y, agent.battery, start);
				idle = false;
			}
		}
	}
	...
	static public void main(String[] passedArgs) {
		String[] appletArgs = new String[] { "GUI_0_95_PartiallyWeightedG" };
		if (passedArgs != null) {
