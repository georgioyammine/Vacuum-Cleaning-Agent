...
public class GUI_1_01 extends PApplet {
	...
	public void setup() {
		...
		start = new Room(x, y);
		Agents.add(new AgentBad(x-1, y-1, 100000));
//		Agents.add(new AgentBad(x-1, y-1, 100000));
//		Agents.add(new AgentBad(x-1, y-1, 100000));
//		Agents.add(new AgentBad(x-1, y-1, 100000));
//		Agents.add(new AgentBad(x-1, y-1, 100000));
	...
	public void startTimer() {
		...
		t.schedule(new TimerTask() {
			...
			public void run() {
				...
				surface.setTitle("[Fully Observable Environement Agents] BFS to Closest Dirt Agent "
...
//			System.out.println("\t"+agent);
			...
			text(Agents.indexOf(agent)+1, (float)(agent.x+0.449)*k, (float)(agent.y +0.4)* k);
			...
			float val = (float) startBtn.getY() + 53 + (105*Agents.indexOf(agent));
...
//		System.out.println(agent.x+" "+agent.y);
	...
	public void updateAgent(int x, int y) {
		Agents.get(dropBox.getSelectedIndex()).setPosition(x, y);
	}
	...
	public void pre() {
		if (pWidth != width || pHeight != height) {
			...
			dropBox.moveTo(nx, ny);
			batteryField.moveTo(nx+28, ny);
			nx = width - wallBtn.getWidth() - 1;
...
//		reset();
	...
	public void createGUI() {
		...
		batteryField = new GTextField(this, 308, 107, 42, 30, G4P.SCROLLBARS_NONE);
		batteryField.setPromptText("Battery");
		...
		dropBox = new GDropList(this, 706, 136, 28, 800,40);
		ArrayList<String> arr = new ArrayList<>();
		for(int i =0;i<Agents.size();i++)
			arr.add(1+i+"");
		dropBox.setItems(arr,arr.size());
	...
	static public void main(String[] passedArgs) {
		String[] appletArgs = new String[] { "GUI_1_01" };
		if (passedArgs != null) {
