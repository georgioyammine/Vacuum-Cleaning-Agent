...
public class GUI_0_95_SA_TSP extends PApplet {
	...
	public void startTimer() {
		...
		t.schedule(new TimerTask() {
			...
			public void run() {
				...
				if (Run) {
					if (!idle) {
						if (pause) {
							...
							int temp = 8-(delayOriginal==0?0:delayOriginal==1?4:delayOriginal==3?7:delayOriginal==7?8:9);
							if(temp==-1) {
								agent.performance-=2;
								agent.battery-=2;
								agent.timeOn+=2;
				...
				surface.setTitle("[Fully Observable Environement Agents] TSP Agent " + nf(appTime / 60, 2) + ":" + nf(appTime == -1 ? 0 : appTime % 60, 2));
				if(pause)
					appTime++;
			}
		...
		try {
			if (start.getNbOfReachableDirt(agent) > 0) {
				...
				if (start.isDirt(agent.x, agent.y)) {
					...
					Tile tile;
					if (pause && start.nbOfDirt() != 0 && results.isEmpty()) {
						pause = false;
	...
	static public void main(String[] passedArgs) {
		String[] appletArgs = new String[] { "GUI_0_95_SA_TSP" };
		if (passedArgs != null) {
